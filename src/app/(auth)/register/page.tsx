        ...
        <div className="grid grid-cols-3 gap-2">
          {(['LEARNER', 'PARENT', 'TEACHER'] as const).map((r) => (
            <button
              ...
              style={{
                background: role === r ? 'var(--primary)' : 'var(--surface-raised)',
                color: role === r ? '#0F1F1A' : 'var(--text-muted)',
              ...
              <div className="grid grid-cols-6 gap-1.5">
                {GRADES.map((g) => (
                  <label key={g} className="cursor-pointer">
                    <input type="radio" name="grade" value={g} className="sr-only peer" required />
                    <span className="flex items-center justify-center rounded-lg border py-1.5 text-sm font-medium transition-colors peer-checked:font-bold"
